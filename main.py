...
async def addrefund(interaction: discord.Interaction, user: discord.User, refund_type: str, item: str = None, amount: int = None, weapon: str = None, ammo: int = None):
    ...
    try:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if cur.rowcount == 0:
                    await interaction.followup.send("❌ Refund niet gevonden of al geclaimd/geannuleerd.", ephemeral=True)
                    return
        # Log
        log_chan = interaction.guild.get_channel(REFUND_LOG_CHANNEL_ID)
        if log_chan:
...
async def refund_informatie_gebruiker(interaction: discord.Interaction, user: discord.User):
    ...
    if not refunds:
        await interaction.followup.send(f"Geen refunds voor {user.mention}.", ephemeral=True)
        return
...
kan je toevoegen in dit refund ssyteem dat ie bij kopjes item de ld_items pakt en laat zien dat je eruit kan kiezen, want daarin zitten 700 item stukken in wat je kan gebruiken ( zit in de databse ) 

voeg ook toe dat je als je de refund wilt geven niet Is dit de refund die je aan @eness wilt geven? weapon weapon_m1911
(nieuwe embed)
Bevestiging
Weet je zeker dat je onderstaande refund aan @eness wilt geven?
(nieuw embed)
Refund Details
Is dit de refund die je aan @eness wilt geven?
Type
Wapen
Wapen
weapon_snspistol
Aantal
1
bevestigen (groen) annuleren (rood)
LD Refund Bot•vandaag om 20:17
maar  andere naam zo een menu is het 

maak ook dat hij een prive pm stuurt als de refund is bevestigd zoals dit

Refund Bevestigd
Je refund is bevestigd door @eness, hieronder vind je de details.
Refund Details
Is dit de refund die je aan @eness wilt geven?
Refund ID
4
...
Volg deze stappen om je refund te claimen:
Ga in-game
Open het refundmenu met /refunds
Claim je refund

LD Refund Bot•gisteren om 15:05
